...
const CartSidebar: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div 
        className={`fixed top-0 right-0 h-full w-full max-w-md bg-background shadow-2xl z-[70] transform transition-transform duration-300 ease-out border-l border-primary/10 flex flex-col ${
          isCartOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
        {/* Header */}
        <div className="p-6 border-b border-primary/10 flex justify-between items-center bg-surface flex-shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-6">
          {items.length === 0 ? (
            ...
            items.map(item => (
              <div key={item.id} className="flex flex-col bg-surface p-4 rounded-2xl shadow-sm border border-primary/5 gap-4">
                <div className="flex gap-4 items-start">
                  <div className="w-20 h-20 rounded-xl overflow-hidden flex-shrink-0 bg-gray-100 border border-primary/10">
                    <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
                  </div>
                  
                  <div className="flex-1">
                    <h3 className="font-serif text-accent font-bold leading-tight mb-1 text-lg">{item.name}</h3>
                ...
                <div className="flex justify-between items-center pt-2 border-t border-primary/5">
                  <div className="flex items-center gap-3 bg-background rounded-full px-2 py-1 border border-primary/10 shadow-sm">
                    <button 
                  ...
                  <button 
                    ...
                    className="text-red-400 hover:text-red-600 hover:bg-red-50 transition-all p-2 rounded-full"
                    title="Remover item"
                  >
                    <Trash2 size={18} />
                  </button>
        ...
        {items.length > 0 && (
          <div className="p-6 bg-surface border-t border-primary/10 space-y-4 flex-shrink-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-20">
